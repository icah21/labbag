import cv2
import numpy as np  # actively used
...
def start_dashboard(detection_queue):
    def show_logo():
        try:
    ...
    def predict_and_update(frame):
        nonlocal last_pred_time, last_predicted_frame

        last_pred_time = time.time()
        resized_frame = cv2.resize(frame, (640, 480))

        # NumPy usage: ensure type and shape
        if not isinstance(resized_frame, np.ndarray):
            resized_frame = np.array(resized_frame, dtype=np.uint8)
        ...
        for pred in predictions.get("predictions", []):
            ...
            # NumPy usage: blur inside detection box
            sub_img = frame[y1:y2, x1:x2]
            blurred = cv2.GaussianBlur(sub_img, (7, 7), 0)
            frame[y1:y2, x1:x2] = np.where(np.array([True]), blurred, sub_img)
        ...
        if counts[detected] > 0:
            detection_queue.put(detected)

        last_predicted_frame = frame

    def update_frame():
        nonlocal last_predicted_frame, camera_ready
        ret, frame = cap.read()
        if ret:
            if not camera_ready:
                camera_ready = True
    ...
    tk.Button(dashboard, text="❌ Exit", font=("Arial", 12), command=root.quit, bg="#FF6347", fg="white", relief="flat", padx=15, pady=5).pack(pady=20)
